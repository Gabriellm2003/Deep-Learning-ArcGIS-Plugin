...
#ROOT_DIR = '/home/tf/erosion/mrcnn/'
print(ROOT_DIR)
...
class ErosionDataset(utils.Dataset):
        ...
        for sid, source in enumerate(source_list):
            ...
            for f in files:
                if '.jpg' in f:
                    name = f.split('.')[0]
                    mask = name+'_mask.jpg'
                    refid = int(re.search('crop(.*)\.jpg', f).group(1))
                    self.add_image("tcu-images", ids, os.path.join(image_dir,f) , width=size, height=size, 
                                    part=part, name=name, mask=mask, refid=refid, 
    ...
    def load_mask(self, image_id):
        ...
        mask_file = info['mask']
        mask = (cv2.imread(os.path.join(mask_source,mask_file), 0) > 0).astype(bool)
...
def getReferencePoints(folders):
    files = [os.path.join(f, "referencepoints.npy") for f in folders]
    pts =  [np.load(f) for f in files if os.path.isfile(f)]
    if len(files) != len(pts):
        print("Error: Not all folders contain reference points to create complete map.")
        return
    return pts

def getReferenceImagesPaths(folders):
    files = [os.path.join(f, "referenceimgspaths.npy") for f in folders]
    paths =  [np.load(f)[0] for f in files if os.path.isfile(f)]
    if len(files) != len(paths):
        print("Error: Not all folders contain reference images paths to save predction as tiff.")
        return
    return paths
...
def main():
    ...
    if process == 'training':
        ...
        TRAINED_MODEL = get_last_weights(prev_model_weights) # placeholder
        MODEL_DIR = os.path.join(ROOT_DIR, 'tcu_model', 'model1')
        ...
        VISUAL_DIR = os.path.join(ROOT_DIR, "tcu_model", outputfolder, "visualization")
        ...
        if complete_vmap:
            ...
            completegts, completepreds, completegtsnames = getGroundTruth(val_folders)
        ...
        for image_id in dataset_val.image_ids:
            ...
            if complete_vmap:
                ...
                metrics.update_complete_visual_map(completepreds[sid], pred, scr, referecepoints, sid, rid)
        ...
        metrics.evaluate(cm_seg, pred=pred, gt=gt)
        print("----------- Detection ----------- ")
        print("Confusion Matrix= " + np.array_str(cm_det).replace("\n", ""))
        print("mAP= ", np.mean(APs))

        if complete_vmap:
            for x in range(len(completepreds)):
                fname = os.path.join(VISUAL_DIR, completegtsnames[x] + "_pred.png")
                fnamecolor = os.path.join(VISUAL_DIR, completegtsnames[x] + "_pred_color.png") 
                vmap = metrics.save_complete_visual_map(completepreds[x], fname)
        ...
        if complete_vmap:
            ...
            _, completepreds, completegtsnames = getGroundTruth(test_folders, testing=True)
            refimgspaths = getReferenceImagesPaths(test_folders)

        model_path = get_last_weights(prev_model_weights) #prev_model_weights
        ...
        shapenames = []
        for x in range(len(completepreds)):
            fname = os.path.join(VISUAL_DIR, completegtsnames[x] + "_pred.tif")
            fnamecolor = os.path.join(VISUAL_DIR, completegtsnames[x] + "_pred_color.png") 
            #vmap = metrics.save_complete_visual_map(completepreds[x], fname, binarize=True)
            vmap = metrics.save_complete_visual_map_as_tif(completepreds[x], fname, refimgspaths[x])
